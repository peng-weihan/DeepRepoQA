...
from DeepRepoQA.completion.model import Completion, StructuredOutput
from DeepRepoQA.exceptions import CompletionRejectError, CompletionRuntimeError
...
class CompletionModel(BaseModel):
    ...
    # Lower-level function for getting model responses called by create_completion
    ...
    def model_validate(cls, obj):
        if isinstance(obj, dict) and "response_format" in obj:
            if "claude-3-5" in obj["model"]:
                from DeepRepoQA.completion.anthropic import AnthtropicCompletionModel
            ...
            if response_format == LLMResponseFormat.TOOLS:
                from DeepRepoQA.completion.tool_call import ToolCallCompletionModel
                return ToolCallCompletionModel(**obj)
            elif response_format == LLMResponseFormat.REACT:
                from DeepRepoQA.completion.react import ReActCompletionModel
                return ReActCompletionModel(**obj)

        return cls(**obj)

    # When the model is configured with a custom base URL (model_base_url) but no API key (model_api_key) is provided, it automatically retrieves the API key from environment variables.
